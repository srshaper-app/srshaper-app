...
export default function FundasPage() {
  ...
  useEffect(() => {
    supabasePublic.from('products').select('*').eq('active', true).eq('category', 'Accesorios')
      .eq('subcategory', 'Fundas').order('created_at', { ascending: false })
  ...
  return (
    <main>
      <section className="page-hero">
        <p className="breadcrumb">Inicio / {t(lang, 'nav_accesorios')} / {t(lang, 'nav_fundas')}</p>
        <h1>{t(lang, 'fundas_h1')}</h1>
        <p className="lead">{t(lang, 'fundas_lead')}</p>
