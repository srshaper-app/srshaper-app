...
export default function GripsPage() {
  ...
  useEffect(() => {
    supabasePublic.from('products').select('*').eq('active', true).eq('category', 'Accesorios')
      .eq('subcategory', 'Grips').order('created_at', { ascending: false })
  ...
  return (
    <main>
      <section className="page-hero">
        <p className="breadcrumb">Inicio / {t(lang, 'nav_accesorios')} / {t(lang, 'nav_grips')}</p>
        <h1>{t(lang, 'grips_h1')}</h1>
        <p className="lead">{t(lang, 'grips_lead')}</p>
